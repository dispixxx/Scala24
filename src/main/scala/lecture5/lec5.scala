...
object ScalaOption extends App {
  ...
  lazy val o1Value_1 = o1.getOrElse {
    println(1111)
    0
  }
...
object ScalaOptionMap extends App {
  ...
  val x1: Option[Int] = o1.map(_ + 1).map(_ + 1).map(_ + 1).map(_ + 1).map { i =>
    println(s"i = $i")
    i + 1
  }
...
object ScalaOptionMapSomeSome extends App {
  ...
  val getValue: Option[Int] => Int = _.getOrElse(0)
  def getValueFunc(option: Option[Int]) : Int = option.getOrElse(0)
...
/*  val x2 = Option(p).flatMap(p => Option(p.id))*/

  /*  def getIdFromOption(elem:Person):Option[Any] = {
    Option(elem).flatMap(elem => Option(elem.name))
    }
    println(s"test: ${getIdFromOption(p)} ")*/
...
object OptionWhen extends App {
  ...
  val o1 = Option.when(cond = true)("true")
  val o2 = Option.when(cond = false)(1)
  println(o1)
  println(o2)
}
